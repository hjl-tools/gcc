...
(define_insn "*call"
  [(call (mem:QI (match_operand:W 0 "call_insn_operand" "<c>BwBz"))
	 (match_operand 1))]
  "!SIBLING_CALL_P (insn)"
  "* return ix86_output_call_insn (insn, operands[0], NULL_RTX);"
  [(set_attr "type" "call")])

;; This covers both call and sibcall since only GOT slot is allowed.
(define_insn "*call_got"
  [(call (mem:QI (match_operand:P 0 "GOT_memory_operand" "Bg"))
	 (match_operand 1))]
  "TARGET_64BIT"
{
  rtx fnaddr = operands[0];
  if (GET_CODE (fnaddr) == ZERO_EXTEND)
    fnaddr = XEXP (fnaddr, 0);
  return ix86_output_call_insn (insn, fnaddr, NULL_RTX);
}
  [(set (attr "type")
     (if_then_else (match_test "(cfun->machine->indirect_branch_type
				 != indirect_branch_keep)")
  ...
  "TARGET_64BIT"
{
  ...
  if (GET_CODE (fnaddr) == ZERO_EXTEND)
    fnaddr = XEXP (fnaddr, 0);
  operands[2] = gen_rtx_REG (Pmode, R11_REG);
  return ix86_output_call_insn (insn, fnaddr, operands[2]);
}
  [(set_attr "type" "multi")])
...
(define_insn "*call_value"
  [(set (match_operand 0)
	(call (mem:QI (match_operand:W 1 "call_insn_operand" "<c>BwBz"))
	      (match_operand 2)))]
  "!SIBLING_CALL_P (insn)"
  "* return ix86_output_call_insn (insn, operands[1], NULL_RTX);"
  [(set_attr "type" "callv")])

;; This covers both call and sibcall since only GOT slot is allowed.
(define_insn "*call_value_got"
  [(set (match_operand 0)
	(call (mem:QI (match_operand:P 1 "GOT_memory_operand" "Bg"))
	      (match_operand 2)))]
  "TARGET_64BIT"
{
  rtx fnaddr = operands[1];
  if (GET_CODE (fnaddr) == ZERO_EXTEND)
    fnaddr = XEXP (fnaddr, 0);
  return ix86_output_call_insn (insn, fnaddr, NULL_RTX);
}
  ...
  "TARGET_64BIT"
{
  ...
  if (GET_CODE (fnaddr) == ZERO_EXTEND)
    fnaddr = XEXP (fnaddr, 0);
  operands[3] = gen_rtx_REG (Pmode, R11_REG);
  return ix86_output_call_insn (insn, fnaddr, operands[3]);
}
  [(set_attr "type" "multi")])
